...
namespace OrderBentoProject.Utility
{
    public class LoginHelper
    {
        ...
        public bool TryLogIn(string Account, string Password)
        {
            ...
            string[] readcol = { "UserID", "Account", "PassWord","Name", "Privilege", "UserPic"};
            string[] Pname = { "@Account" };
            string[] P = { Account };
            var getaccount = new DBTool();
            List<User> dtuserAccount = getaccount.ChangeTypeUserInfo(getaccount.readTable("Users", readcol, "WHERE Account=@Account", Pname, P));
            if (dtuserAccount.Count == 0)
                return false;
            User userAccount = dtuserAccount[0];
